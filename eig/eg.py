...
from xtf import xTF, iSTF, STF, MTF
...
smax = np.abs(np.max(s))
ii = [ i for i, val in enumerate(s) if val > tolrank * smax ]
...
plt.figure()
plt.plot(Psi.real, Psi.imag, 'b.-')
plt.plot(w[iind].real, w[iind].imag, 'ro', label='computed')

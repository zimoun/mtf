...
def evalIncDirichletTraceInt(point, normal):
    x, y, z = point
    return np.exp( 1j * kInt * (x))
...
incDirichletTraceInt = createGridFunction(
    context, space, space, evalIncDirichletTraceInt,
    surfaceNormalDependent=True)

# Extract plane wave

exportToGmsh(incDirichletTrace, 'inc-dext', 'Inc_De.pos')
exportToGmsh(incDirichletTraceInt, 'inc-dint', 'Inc_Di.pos')
...
V0 = createLaplace3dSingleLayerBoundaryOperator(
    context, space, space, space, "V0")
K0 = createLaplace3dDoubleLayerBoundaryOperator(
    context, space, space, space, "K0")
K0a = createLaplace3dAdjointDoubleLayerBoundaryOperator(
    context, space, space, space, "Q0")
W0 = createLaplace3dHypersingularBoundaryOperator(
    context, space, space, space, "W0")
V1 = createLaplace3dSingleLayerBoundaryOperator(
    context, space, space, space, "V1")
K1 = createLaplace3dDoubleLayerBoundaryOperator(
    context, space, space, space, "K1")
K1a = createLaplace3dAdjointDoubleLayerBoundaryOperator(
    context, space, space, space, "Q1")
W1 = createLaplace3dHypersingularBoundaryOperator(
    context, space, space, space, "W1")
...
STFsolver = createDefaultIterativeSolver(STF)
STFsolver.initializeSolver(defaultGmresParameterList(1e-8))
...
print STFsolution.iterationCount()
...
STFintDirichletTrace = STFextDirichletTrace - incDirichletTrace
STFintNeumannTrace = -STFextNeumannTrace  - incNeumannTrace

t =  STFextDirichletTrace
exportToGmsh(t, 'STF-dext-r', 'STF_DextR.pos')
tI = -1j * t
exportToGmsh(tI, 'STF-dext-i', 'STF_DextI.pos')

t =  STFintDirichletTrace
exportToGmsh(t, 'STF-dint-r', 'STF_DintR.pos')
tI = -1j * t
exportToGmsh(tI, 'STF-dint-i', 'STF_DintI.pos')

# t =  STFscattDirichletTrace
# exportToGmsh(t, 'STF-dsca-r', 'STF_DscaR.pos')
# tI = -1j * t
# exportToGmsh(tI, 'STF-dsca-i', 'STF_DscaI.pos')

# t =  STFscattNeumannTrace
# exportToGmsh(t, 'STF-nsca-r', 'STF_NscaR.pos')
# tI = -1j * t
# exportToGmsh(tI, 'STF-nsca-i', 'STF_NscaI.pos')
...
t =  MTFextDirichletTrace
exportToGmsh(t, 'MTF-dext-r', 'MTF_DextR.pos')
tI = -1j * t
exportToGmsh(tI, 'MTF-dext-i', 'MTF_DextI.pos')

t =  MTFintDirichletTrace
exportToGmsh(t, 'MTF-dint-r', 'MTF_DintR.pos')
tI = -1j * t
exportToGmsh(tI, 'MTF-dint-i', 'MTF_DintI.pos')
...
normDext = MTFextDirichletTrace.L2Norm()
normNext = MTFextNeumannTrace.L2Norm()
normDint = MTFintDirichletTrace.L2Norm()
normNint = MTFintNeumannTrace.L2Norm()

normDinc = incDirichletTrace.L2Norm()
normNinc = incNeumannTrace.L2Norm()

eMSDext = (MTFextDirichletTrace - STFextDirichletTrace).L2Norm()
eMSNext = (MTFextNeumannTrace - STFextNeumannTrace).L2Norm()

jD = (MTFextDirichletTrace - MTFintDirichletTrace -incDirichletTrace).L2Norm()
jN = (-MTFextNeumannTrace - MTFintNeumannTrace -incNeumannTrace).L2Norm()
...
print('')
print("# error  {:^15} {:^15}".format(
    'jumpD', 'jumpN'
    ))
print("# MTF    {:^15E} {:^15E}".format(
    jD, jN
))
print('')
...
#     [ZZ, Half, ZZ, Id] 
#     ]
# IIblock = createBlockedBoundaryOperator(context, a)
# tt = time.time()
# print(tt-t)
...
# MM = np.array([])
# for ops in MTF_list:
#     temp = ops[0].weakForm().asMatrix()
#     for op in ops[1:]:
#         a = op.weakForm().asMatrix()
#         temp = np.concatenate((temp, a), axis=1)
#     if MM.shape == (0,):
#         MM = temp
#     else:
#         MM = np.concatenate((MM, temp), axis=0)
# tt = time.time()
# print(tt-t),
...
#         SS = temp
#     else:
#         SS = np.concatenate((SS, temp), axis=0)
# tt = time.time()
# print(tt-t),

# S = STF.weakForm().asMatrix()
# bstf = np.array([])
# for v in rhsSTF:
#     bstf = np.concatenate((bstf, v.projections()))
# tt = time.time()
# print(tt-t),
# print('done.\n')
# xstf = np.linalg.solve(S, bstf)

# yd = STFextDirichletTrace.coefficients()
# yn = STFextNeumannTrace.coefficients()
# ystf = np.concatenate((yd, yn))
...
# # II = Id.weakForm().asMatrix()
# # KK = K1.weakForm().asMatrix()
# # VV = V1.weakForm().asMatrix()
# # WW = W1.weakForm().asMatrix()
# # QQ = K1a.weakForm().asMatrix()

# # print('==== end weakForm.')
# # sys.stdout.flush()

# # gdk = (0.5*Id + K1) * incDirichletTrace
# # gdv = -V1 * incNeumannTrace
# # gnw = W1 * incDirichletTrace
# # gnq = (-0.5*Id+K1a) * incNeumannTrace

# # gd = gdk + gdv
# # gn = gnw + gnq

# # print('==== end assembling.')
# # sys.stdout.flush()

# # cgdk = gdk.coefficients()
# # cgdv = gdv.coefficients()
# # cgnw = gnw.coefficients()
# # cgnq = gnq.coefficients()

# # print('==== end coefficients.')
# # sys.stdout.flush()

# # pgdk = gdk.projections()
# # pgdv = gdv.projections()
# # pgnw = gnw.projections()
# # pgnq = gnq.projections()

# # print('==== end projections.')
# # sys.stdout.flush()

# # exportToGmsh(gdk, 'gdk', 'gdk.pos')
# # exportToGmsh(gdv, 'gdv', 'gdv.pos')
# # exportToGmsh(gnw, 'gnw', 'gnw.pos')
# # exportToGmsh(gnq, 'gnq', 'gnq.pos')

# # exportToGmsh(gnw, 'gd', 'gd.pos')
# # exportToGmsh(gnq, 'gn', 'gn.pos')


# # print('==== end export.')
# # sys.stdout.flush()

# # cgd = incDirichletTrace.coefficients()
# # cgn = incNeumannTrace.coefficients()

# # pgd = incDirichletTrace.projections()
# # pgn = incNeumannTrace.projections()

# # print('==== end projection Inc.')
# # sys.stdout.flush()

# # cvgdk = (0.5*II + KK).dot(cgd)
# # cvgdv = VV.dot(cgn)
# # cvgnw = WW.dot(cgd)
# # cvgnq = (-0.5*II+QQ).dot(cgn)

# # pvgdk = (0.5*II + KK).dot(pgd)
# # pvgdv = VV.dot(pgn)
# # pvgnw = WW.dot(pgd)
# # pvgnq = (-0.5*II+QQ).dot(pgn)
